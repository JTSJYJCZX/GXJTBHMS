...
{
    public class CableForceRealTimeDatasService : ServiceBase, ICableForceRealTimeDatasService
    {
        ICableForceRealTimeDatasDAL _cfdDAL;
        IMonitoringPointsNumberDAL _mpnDAL;
        IMonitoringPointsPositionDAL _mppDAL;
        public CableForceRealTimeDatasService(ICableForceRealTimeDatasDAL cfdDAL, IMonitoringPointsNumberDAL mpnDAL, IMonitoringPointsPositionDAL mppDAL)
        {
            _cfdDAL = cfdDAL;
            _mpnDAL = mpnDAL;
            _mppDAL = mppDAL;
        ...
        public IEnumerable<IncludeSectionWarningColorDataModel> GetWarningCableForceDatasBy(IEnumerable<int> sectionIds)
        {
            ...
            try
            {
                foreach (var item in sectionIds)
                {
                    ...
                    var source = _cfdDAL.GetRealTimeCableForce(item).ToArray();
        ...
        private List<RealTimeWarningDataModel> GetResultsOfOneSection(CableForceTable[] source)
        {
            ...
            for (int i = 0; i < source.Length; i++)
            {
                ...
                pointModel.CurrentData = source[i].CableForce;
