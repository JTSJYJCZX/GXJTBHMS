...
{
    public class HumidityDatasDAL : Repository<HumidityTable, int>, IHumidityDatasDAL
    {
        public override IEnumerable<HumidityTable> FindBy(IList<Func<HumidityTable, bool>> ps, int currentPageIndex, int pageSize, params string[] navigationProperties)
        {
            ...
            {
                var source = DealWithNavigationPropertys(navigationProperties, ctx.Humiditys);//处理导航属性
